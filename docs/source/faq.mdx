...
As for the GPU accelerator, once the average GPU utilization of all your endpoints averaged over a 2 minute window reaches 80%, a new endpoint will be added. One Endpoint can be scaled up every 3 minutes and scale down time is 5 minutes. Please note that if your model requires a significant amount of time to initialize, autoscaling may not be effective. This is because the average GPU utilization may fall below the threshold, causing your endpoint to be automatically scaled down.
