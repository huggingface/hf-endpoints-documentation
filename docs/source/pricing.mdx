...
The pricing below shows public on-demand pricing for Inference Endpoints. You or your organization will only be billed while your Endpoint is running. While the prices are by the hour, the actual cost will be calculated by the minutes.
...
- Autoscaling:
  - Min replica: 1
  - Max replica: 3
  - Every hour, a spike of load makes the Endpoint automatically scale from 1 to 3 replicas for a duration of 15 minutes

**Cost calculation:**

`Instance hourly rate * ((hours * # min replica) + (scale-out hours * # additional replicas))`

**Hourly cost:**

`$0.6/h * ((1h * 1) + (0.25h * 2)) = $0.9/hour`
