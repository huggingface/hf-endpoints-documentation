...
A: It depends on the Task. The [supported Tasks](/supported_tasks) are using the transformers or sentence-transformers pipelines under the hood. If your Task pipeline supports batching, e.g. Zero-Shot Classification then batch inference is supported. In any case, you can always create your own [inference handler](/guides/custom_handler) and implement batching. 
