...
You can change your scaling criteria to be based on pending request rather than utilization metrics. This is still an experimental feature, which means that it might change and we don't recommend using it for production workloads.

- Pending requests are requests that don't yet have an http status, meaning in-flight requests and requests being processed
- If there are more than 1.5 pending requests per replica in the last 20 seconds, it triggers an autoscaling event, and adds a replica to your deployment
