...
You can change the scaling criteria to be based on pending requests instead of utilization metrics. This is currently an experimental feature and we advice caution if you're using it for production workloads.
...
- by default, if there are more than 1.5 pending requests per replica in the past 20 seconds, it triggers an autoscaling event and adds a replica to your deployment.
    - you can adjust this threshold to meet your specific requirements from the settings page.
