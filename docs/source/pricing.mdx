...
Below are two pricing example for a simple endpoint and more advanced endpoint.
...
`Instance type * ((hours * # min replica) + (scale-out hours * # additional replica))`
...
The pricing below includes the public on-demand pricing for endpoints. You will only be billed while your endpoint is running.
If you are interested in custom pricing, please [contact us](https://huggingface.co/inference-endpoints/enterprise).
