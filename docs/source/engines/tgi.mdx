...
By default the TGI version will be the latest available one (with some delay). But you can also specify a differen version by [chaning
...
- **Max Input Tokens (per query)**: The maximum number of input tokens, meaning the amount of tokens in the 
