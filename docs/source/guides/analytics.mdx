...
The first graph to the top left shows you how many requests your Inference Endpoint has received. By default they are grouped by HTTP response
...
being processed. If this metrics increases too much it means that you're requests are queing up, and your users have to wait for requests
...
![pending](https://raw.githubusercontent.com/huggingface/hf-endpoints-documentation/update/large-rewrite/assets/analytics/4-pendging-reqs.png)
...
In the replica status graph you'll see in the basic view how many running replicas you during a point in time. And the red line shows
what currently is your maximum replicas. 
