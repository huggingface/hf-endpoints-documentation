...
serving of open-source LLMs (e.g. LLaMA, Falcon, StarCoder, BLOOM and many more). If a model is supported by TGI it's one of the
recommended choices. The core features that make TGI a good choice are:
...
![config](https://raw.githubusercontent.com/huggingface/hf-endpoints-documentation/update/large-rewrite/assets/tgi/tgi-config.png)
...
- **Max Input Length (per query)**: This is the same as `Max Number of Tokens` but a legacy naming supported for backwards compatibility. 
...
you can only serve one requests at a time.
...
for the max input lenght, max number of tokens, max batch prefill tokens and max batch total tokens. This means that you'll use

Note that there's caveat: say you're deploying `meta-llama/Llama-3.3-70B-Instruct`, which has context lenght of 128k tokens.
But you're on a GPU where you can only fit the model's context three times in memory. So if you want to serve the model with full context lenght,
you can only serve up to 3 concurrent requests. In some cases, it's fine to drop the maximum context lenght to 64k tokens, which would
...
- A selection of popular models the [Inference Endpoints Catalog](https://endpoints.huggingface.co/huggingface/catalog)
