...
The following pricing scenarios demonstrate how costs are calculated for a simple Endpoint and a more advanced Endpoint with autoscaling.

| Provider | Region    | Instance type           | Autoscaling                                                                                                                          | Cost                                                                                      | Hourly cost | Monthly cost |
|----------|-----------|-------------------------|--------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------|-------------|--------------|
| aws      | us-east-1 | CPU - medium ($0.12/hr) | - min replica: 1<br> - max replica: 1                                                                                                | instance hourly rate * (hrs * # min replica)                                              | $0.12       | $87.6        |
| aws      | us-east-1 | GPU - small ($0.6/hr)   | - min replica: 1<br> - max replica: 3<br> - every hr a load spike automatically scales the Endpoint from 1 to 3 replicas for 15 mins | instance hourly rate * ((hrs * # min replica) + (scale-up hours * # additional replicas)) | $0.9        | $657         |
