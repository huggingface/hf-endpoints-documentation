...
A: It depends on the Task. The [supported Tasks](#supported-tasks) are using the transformers or sentence-transformers pipelines under the hood. If your Task pipeline supports batching, e.g. Zero-Shot Classification then batch inference is supported. In any case, you can always create your own [inference handler](#create-your-own-inference-handler) and implement batching. 
...
A: You can currently monitor your Endpoint through the [🤗 Inference Endpoints web application](https://ui.endpoints.huggingface.co/endpoints), where you have access to the [Logs of your Endpoints](#access-and-read-logs) as well as a [metrics dashboard](#access-and-read-logs). If you need programmatic access or more information, please contact us.
...
A: You can invalidate existing personal tokens and create new ones in your settings here: [https://huggingface.co/settings/tokens](https://huggingface.co/settings/tokens). For organization tokens, go to the organization settings.w